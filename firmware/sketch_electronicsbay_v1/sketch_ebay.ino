...
AltSoftSerial loraSerial; //tx, rx
...
float alt_mpl = 0.0; //create altitude variable (in meter) (MPL3115A2)
...
{ 
  ...
  loraSerial.begin(9600); // connect LORA transceiver
...
{ 
  ...
  if (myIMU.readByte(MPU9250_ADDRESS, INT_STATUS) & 0x01)
  {
    ...
    myIMU.ay = (float)myIMU.accelCount[1] * myIMU.aRes + 1.0; // - myIMU.accelBias[1];
  ...
  */
  ...
  pressure_mpl = readPressure();
  temperature_mpl = readTemp();
  alt_mpl = 44330.77*(1-pow(pressure_mpl/101326.0, 0.1902632))-17.0;
  ...
  loraSerial.print('<');
  loraSerial.print(cycle);
  loraSerial.print(",");
  loraSerial.print(temperature_mpl);
  loraSerial.print(",");
  loraSerial.print(pressure_mpl);
  loraSerial.print(",");
  loraSerial.print(alt_mpl, 2);
  loraSerial.print(",");
  loraSerial.print(lat*100000);
  loraSerial.print(",");
  loraSerial.print(lon*100000);
  loraSerial.print(",");
  loraSerial.print(alt_gps, 6);
  loraSerial.print(",");
  loraSerial.print(gps.satellites());
  loraSerial.print('>'); 

  /*
  Serial.println("In ack loop");
    
  while(ack==false)
  {     
    if(loraSerial.available()>0)
    {
       Serial.println("In loraSerial part");
       recvEndMarker();
       i++;
    }
    ...
    {
      //Serial.println("In i part");
      i++;
    }

    else
    {
       Serial.println("In forced ack part");
       ack=true;
    }
...
void recvEndMarker() 
{
    ...
    if (rc != endMarker) 
    {
       ...
       //Serial.println("Char not yet received");
    }
